...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      // ✅ FIXED — uses environment variable
      const res = await axios.post(`${API_BASE}/auth/login-otp`, {
      ...
      <div className="hidden lg:flex w-1/2 bg-black flex-col justify-between p-12">
        ...
        <div className="flex gap-3 flex-wrap">
          {["Orders", "Invoices", "Payments", "Customers"].map((t) => (
            <span key={t}
              className="text-xs text-white/30 border border-white/10
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase
                tracking-wide mb-1.5 block">Username</label>
              <div className="relative">
                <span className={`absolute left-3 top-1/2 -translate-y-1/2
                  text-black font-bold text-base transition-all duration-300
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase
                tracking-wide mb-1.5 block">Mobile Number</label>
              <div className="relative">
                <span className={`absolute left-3 top-1/2 -translate-y-1/2
                  text-black font-bold text-base transition-all duration-300
            ...
            {otpSent && (
              <div className="animate-fadeIn">
                <label className="text-xs font-semibold text-gray-500 uppercase
                  tracking-wide mb-1.5 block">Enter OTP</label>
                <div className="relative">
                  <span className={`absolute left-3 top-1/2 -translate-y-1/2
                    text-black font-bold text-base transition-all duration-300
            ...
            {!otpSent ? (
              <button onClick={handleSendOtp} disabled={loading}
                className="relative w-full py-3 rounded-xl bg-black text-white
                  font-semibold text-sm overflow-hidden group transition-all
                  duration-300 hover:shadow-lg disabled:opacity-50 mt-2">
              ...
              <div className="space-y-3">
                <button onClick={handleLogin} disabled={loading}
                  className="relative w-full py-3 rounded-xl bg-black text-white
                    font-semibold text-sm overflow-hidden group transition-all
                    duration-300 hover:shadow-lg disabled:opacity-50">
                  <span className="absolute inset-0 bg-white scale-x-0
                    group-hover:scale-x-100 origin-left transition-transform
                    duration-300 rounded-xl" />
                  <span className="relative z-10 group-hover:text-black
          ...
          <p className="mt-8 text-center text-sm text-gray-400">
            ...
            <Link to="/register"
              className="text-black font-semibold hover:underline
                underline-offset-2">Register</Link>
          </p>
        </div>
      </div>
    </div>
