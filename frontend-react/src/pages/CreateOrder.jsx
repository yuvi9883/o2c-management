...
export default function CreateOrder() {
  ...
  useEffect(() => {
    getCustomers()
      .then((res) => setCustomers(res.data.filter((c) => c.status === "ACTIVE")))
      .catch(() => {});
  }, []);
  ...
  const Arrow = ({ id }) => (
    <span className={`absolute left-3 top-1/2 -translate-y-1/2 text-black font-bold text-base transition-all duration-300
  ...
  return (
    <div className="animate-fadeIn max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3 mb-2">
        <button onClick={() => navigate("/orders")} className="text-gray-400 hover:text-black transition-colors">
          ← Back
        </button>
      ...
      {error && (
        <div className="px-4 py-3 bg-red-50 border border-red-100 rounded-xl text-sm text-red-600 flex items-start gap-2">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-4">
          <h3 className="font-bold text-black text-sm uppercase tracking-wide mb-4">Order Details</h3>

          {/* Customer */}
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Customer</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Unit Price</label>
              <input
                readOnly
            ...
            <div>
              ...
              <input
                readOnly
                value={`₹${total}`}
                className="w-full py-2.5 px-4 rounded-xl border border-gray-100 bg-gray-50 text-sm font-bold text-black"
              />
            </div>
          </div>
        </div>
        ...
        {/* Shipping */}
        <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-4">
          <h3 className="font-bold text-black text-sm uppercase tracking-wide mb-4">Shipping Address</h3>
          {[
        ...
        <div className="flex gap-3">
          <button
            type="submit"
            disabled={loading}
            className="relative flex-1 py-3 rounded-xl bg-black text-white font-semibold text-sm overflow-hidden group transition-all duration-300 hover:shadow-lg disabled:opacity-50"
          >
            <span className="absolute inset-0 bg-white scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 rounded-xl"></span>
